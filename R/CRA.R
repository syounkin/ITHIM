CRA <- function(meanlog.baseline = NA, sdlog.baseline = 1, meanlog.scenario = meanlog.baseline, sdlog.scenario = sdlog.baseline, n = 5, B = 1e3, P, Q, R = function(x) exp(-x), type = "parametric"){
    ...
    if(type == "parametric"){
        ...
        TA.baseline <- ifelse(runif < p0, 0, TA.sample)
        ...
        TA.scenario <- ifelse(runif < p0, 0, TA.sample)
