...
ITHIM <- within(ITHIM, {
quintMat <- matrix(computeQuintiles(meanActiveTransportTime, sdActiveTransportTime), nrow = 2*nrow(meanActiveTransportTime), ncol = 5)
quintList = list(M = quintMat[1:ITHIM$parList.nAgeClass,], F = quintMat[ITHIM$parList.nAgeClass+1:8,])
})

ITHIM <- within(ITHIM, {
propTimeCycling = propTimeCycling <- meanCycleTime/(meanCycleTime+meanWalkTime)
})
