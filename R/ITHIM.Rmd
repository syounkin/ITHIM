...
```{r VS11}
tapePosition <- list(sex = 69, ageUnit = 70, ageValue = c(71:73)) # Magic number!!!
colClasses <- c("factor", "factor","integer")
fileWidth <- 488 # Magic number!!!

cbind(names(tapePosition), colClasses)
...
```{r convertAge, warning = FALSE}

varWidths <- unlist(lapply(tapePosition, "length"))
widths <- c(-(tapePosition[[1]]-1),c(varWidths))
widths <- c(widths,-(fileWidth-sum(abs(widths))))

Data.Mortality <- read.fwf(file = "~/ITHIM/R/data/VS11MORT.DUSMCPUB", widths = widths, col.names = names(tapePosition), header = FALSE, colClasses = colClasses)
