...
```{r vitalstats, eval = FALSE, echo = TRUE}
source("../vargo/process_2001_vital_stats.r")
saveRDS(mortality2001, file = "./data/mortality2001.rds")
```

```{r histAge, eval = FALSE, echo = TRUE}
mortality2001 <- readRDS(file = "./data/mortality2001.rds")
```
...
```{r VS11}
tapePosition <- list(sex = 69, ageUnit = 70, ageValue = c(71:73), skippedVars = c(74:145), ICD = 146:149) # Magic number!!!
colClasses <- c("factor", "factor","integer", "character")
...
```{r ageClass}
ageClass <- c(rep("0-4",5),rep("5-14",10),rep("15-29",15),rep("30-44",15),rep("45-59",15),rep("60-69",10),rep("70-79",10),rep("80+",121))
...
D <- data.frame(ageClass = Data.Mortality$ageClass)
p <- ggplot(D, aes(ageClass))
p + geom_bar()
```
